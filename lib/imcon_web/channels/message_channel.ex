defmodule ImconWeb.MessageChannel do
  ...
  alias ImconWeb.{MessageService, MessageView, UnreadService, EventChannel, ChannelUserService}
  ...
  def handle_in("new_message", %{"text" => _text} = params, user, socket) do
    channel = channel_from_topic(socket.topic)
    if channel do
      changeset = Chat.message_changeset(%Message{}, message_params(params, channel, user))
  ...
  defp ensure_opposite_user_joined(user, channel) do
    opposite_id = Chat.opposite_direct_user_id(channel, user.id)
    {:ok, channel_user} = ChannelUserService.rejoin_channel(opposite_id, channel)
    channel_user
  end
