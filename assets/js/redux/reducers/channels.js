...
function getDirectChannelIdByName(channels, user) {
  return _.transform(channels, (result, channel, id) => {
    result[user[channel.userId].username] = channel.id
  })
}
  ...
  switch (action.type) {
    ...
    case types.FETCH_DIRECT_CHANNELS_SUCCESS:
      ...
      return {
        ...
        channelIdByName: {
          ...state.channelIdByName,
          ...getDirectChannelIdByName(action.response.entities.channels, action.payload.user)
        }
      }
      break
    ...
    case types.JOIN_DIRECT_CHANNEL_SUCCESS:
      ...
      return {
        ...
        channelIdByName: {
          ...state.channelIdByName,
          ...getDirectChannelIdByName(channels, action.payload.user)
        }
      }
      break
